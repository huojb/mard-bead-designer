...
> **v0.3.0 更新：** 现在支持动态调整输出网格尺寸，不再强制缩放到 52×52。您可以根据实际拼豆设计需要，选择合适的目标尺寸（如 18×18、24×36 等），大幅减少过度下采样导致的信息损失。

图片导入提供以下调节参数：
- **输出网格尺寸** — 自动模式（设长边 bead 数）/ 手动指定宽高，支持保持原图比例
  - 快捷预设：极小(12) / 小型(18) / 标准(24) / 中型(32) / 大型(48)
- **插值方式** — 平滑（Bilinear，默认） / 硬边缘（Nearest Neighbor）
